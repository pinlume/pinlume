...
class ScreenCaptureManager {
    ...
    /// Synchronous WindowServer snapshot used by global hotkeys before Pinlume
    ...
    static func captureAllScreensImmediatelySCK(
        ...
        if excludingWindowNumbers.isEmpty, #available(macOS 26.0, *) {
        ...
        let excludedIDs = Set(excludingWindowNumbers)
        ...
        let captures = await withTaskGroup(
            ...
            for (index, pair) in pairs.enumerated() {
                ...
                group.addTask {
                    // Capture the whole display. Ordinary captures preserve all
                    // transient UI; a recording-originated capture filters only
                    // its persistent border/HUD chrome from the output image.
    ...
    static func captureAllScreens(
        ...
        Task {
            do {
                ...
                #if DEBUG
                    NSLog("Pinlume: screen capture error: \(error.localizedDescription)")
                #endif
    ...
    /// On macOS 14.2+, uses `desktopIndependentWindow` for an isolated window image.
    /// Older systems use the one-window CoreGraphics array API — unlike
    /// `.optionIncludingWindow`, it does not composite windows above the target.
    static func captureWindow(windowID: CGWindowID, screen: NSScreen) async -> CGImage? {
        func captureViaWindowList() -> CGImage? {
            CGImage(
                ...
                windowArray: [NSNumber(value: windowID)] as CFArray,
        ...
        if #available(macOS 14.2, *) {
            ...
            // macOS 12.3–14.1: explicitly request only this window ID.
