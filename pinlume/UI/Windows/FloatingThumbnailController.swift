...
extension NSImage {
    func pinlumeTransformed(_ transform: ImageContextTransform) -> NSImage? {
...
class FloatingThumbnailController: NSObject, NSDraggingSource, QLPreviewPanelDataSource, QLPreviewPanelDelegate {
    ...
    /// Editable source data stays with the history entry for later recovery.
    ...
    func show(at origin: NSPoint, corner: FloatingThumbnailCorner, on screen: NSScreen) {
        self.corner = corner
    ...
    @objc private func contextTransform(_ sender: NSMenuItem) {
        guard let transform = ImageContextTransform(rawValue: sender.tag),
              let transformed = image.pinlumeTransformed(transform) else { return }
