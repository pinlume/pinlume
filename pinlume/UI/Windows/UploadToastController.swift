...
class UploadToastController {
    ...
    func show(status: String) {
        showStatus(status, showsSpinner: true, autoDismissSeconds: nil)
    }

    func showMessage(status: String, symbolName: String = "checkmark.circle.fill") {
        showStatus(status, showsSpinner: false, autoDismissSeconds: 2.6, symbolName: symbolName)
    }

    /// Matches OverlayView.showOverlayStatus so a workflow that closes its
    /// overlay (OCR copy) still gives the same visual confirmation as screen
    /// translation copy.
    func showOverlayStyleMessage(status: String) {
        let attributes: [NSAttributedString.Key: Any] = [
            .font: NSFont.systemFont(ofSize: 13, weight: .medium),
        ]
        let textSize = (status as NSString).size(withAttributes: attributes)
        ...
        panel.contentView = OverlayStyleToastView(text: status, padding: padding)
        window = panel
        panel.orderFrontRegardless()
        scheduleDismiss(seconds: 2.0, animated: false)
    }
    ...
    private func showStatus(
        ...
        if showsSpinner {
            ...
            icon.contentTintColor = ToolbarLayout.accentColor
        }
        ...
        if let autoDismissSeconds {
            scheduleDismiss(seconds: autoDismissSeconds)
        }
    }
    ...
    private func scheduleDismiss(seconds: Double, animated: Bool = true) {
        dismissTask?.cancel()
        let task = DispatchWorkItem { [weak self] in
            guard let self else { return }
            animated ? animateOut() : dismiss()
        }
        dismissTask = task
...
private class ToastBackgroundView: NSView {
    ...
    override func mouseDown(with event: NSEvent) {
        ...
        let buttons = subviews.filter { $0 is NSButton }
...
private final class OverlayStyleToastView: NSView {
