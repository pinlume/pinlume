...
class ToolbarButtonView: NSView {
    ...
    /// Key: "symbolName|isOn|colorRGBA". The toolbar has a finite symbol set,
    /// but an explicit LRU ceiling also bounds future dynamic tint variants.
    ...
    private static func cachedIcon(forKey key: String) -> NSImage? {
        guard let image = iconCache[key] else { return nil }
        iconCacheOrder.removeAll { $0 == key }
        iconCacheOrder.append(key)
        return image
    }

    private static func storeIcon(_ image: NSImage, forKey key: String) {
        iconCache[key] = image
        iconCacheOrder.removeAll { $0 == key }
        iconCacheOrder.append(key)
        while iconCache.count > iconCacheLimit, !iconCacheOrder.isEmpty {
    ...
    override func draw(_ dirtyRect: NSRect) {
        ...
        if isSeparator {
            ...
            NSRect(x: bounds.midX - 0.5, y: bounds.minY + 7, width: 1, height: bounds.height - 14).fill()
            return
        }
        ...
        if let swatch = swatchColor {
            ...
            swatch.withAlphaComponent(isEnabled ? 1 : 0.35).setFill()
            NSBezierPath(roundedRect: r, xRadius: 4, yRadius: 4).fill()
            ToolbarLayout.iconColor.withAlphaComponent(isEnabled ? 0.4 : 0.14).setStroke()
        ...
        if cachedIcon == nil || cachedIconIsOn != currentIsOn {
            ...
            if let cached = Self.cachedIcon(forKey: key) {
                cachedIcon = cached
        ...
        if let icon = cachedIcon {
            ...
            icon.draw(at: NSPoint(x: x, y: y), from: .zero, operation: .sourceOver, fraction: isEnabled ? 1.0 : 0.32)
        }
    ...
    override func updateTrackingAreas() {
        ...
        // `.activeAlways` (not `.activeInActiveApp`): Pinlume is a menu-bar
    ...
    override func mouseEntered(with event: NSEvent) {
        NSCursor.arrow.set()
        guard !isSeparator, isEnabled else { return }
