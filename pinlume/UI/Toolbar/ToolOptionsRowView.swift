...
class ToolOptionsRowView: NSView {
    ...
    private enum StampStyleControlTag {
        static let quickEmojiBase = 9510
        static let moreEmoji = 9520
        static let image = 9521
    }
    ...
    /// Update only the inexpensive numeric label while a text size handle is
    /// tracking. Rebuilding the whole row on every mouse event causes visible lag.
    func updateTextFontSizeDisplay(_ size: CGFloat) {
        (viewWithTag(998) as? NSTextField)?.stringValue = "\(Int(size.rounded()))"
    }
    ...
    /// Synchronize stamp style selection without rebuilding the whole row.
    func refreshStampStyleSelection() {
        guard let ov = overlayView,
              currentTool == .stamp || editingAnnotation?.tool == .stamp else { return }
        let quickIndex = ov.currentStampEmoji.flatMap { StampEmojis.common.firstIndex(of: $0) }
        for index in StampEmojis.common.indices {
            applyStampStyleSelection(
                viewWithTag(StampStyleControlTag.quickEmojiBase + index) as? NSButton,
                isSelected: quickIndex == index
            )
        }
        applyStampStyleSelection(
            viewWithTag(StampStyleControlTag.moreEmoji) as? NSButton,
            isSelected: ov.currentStampEmoji != nil && quickIndex == nil
        )
        applyStampStyleSelection(
            viewWithTag(StampStyleControlTag.image) as? NSButton,
            isSelected: ov.currentStampEmoji == nil && ov.currentStampImage != nil
        )
    }

    private func applyStampStyleSelection(_ button: NSButton?, isSelected: Bool) {
    ...
    func rebuild(for tool: AnnotationTool) {
        ...
        // ── Censor tool: manual shape/style/strength + automatic redact buttons ──
        if tool == .pixelate {
            curX = addCensorShapeSegment(at: curX, ov: ov)
            curX = addSeparator(at: curX)
            curX = addCensorStyleSegment(at: curX, ov: ov)
            curX = addSeparator(at: curX)
            curX = addCensorStrengthSlider(at: curX, ov: ov)
            curX = addSeparator(at: curX)
    ...
    private func addStrokeSlider(at x: CGFloat, tool: AnnotationTool, ov: OverlayView) -> CGFloat {
        ...
        if tool == .loupe {
            if let ann = editingAnnotation {
                ...
                currentVal = ov.activeStrokeWidthForTool(tool)
            }
        } else {
            let storedValue = editingAnnotation?.strokeWidth ?? ov.activeStrokeWidthForTool(tool)
            currentVal = AnnotationStrokeWidth.clamped(storedValue)
        }
        let sliderW: CGFloat = tool == .loupe ? 100 : 72
        let slider = NSSlider(value: Double(currentVal),
                              minValue: tool == .loupe
                                ? 40
                                : Double(AnnotationStrokeWidth.minimum),
                              maxValue: tool == .loupe
                                ? 320
                                : Double(AnnotationStrokeWidth.maximum),
    ...
    private func addCensorShapeSegment(at x: CGFloat, ov: OverlayView) -> CGFloat {
        var curX = x
        let seg = NSSegmentedControl()
        seg.segmentCount = 2
        seg.trackingMode = .selectOne
        seg.target = self
        seg.action = #selector(censorShapeChanged(_:))
        (seg.cell as? NSSegmentedCell)?.segmentStyle = .roundRect
        seg.setImage(NSImage(systemSymbolName: "rectangle.dashed", accessibilityDescription: L("Rectangle")), forSegment: 0)
        seg.setImage(NSImage(systemSymbolName: "paintbrush", accessibilityDescription: L("Brush")), forSegment: 1)
        seg.setWidth(34, forSegment: 0)
        seg.setWidth(34, forSegment: 1)
        let shape = editingAnnotation?.censorShape
            ?? CensorShape(rawValue: UserDefaults.standard.integer(forKey: "censorShape"))
            ?? .rectangle
        seg.selectedSegment = shape.rawValue
        seg.toolTip = "\(L("Rectangle")) / \(L("Brush"))"
        seg.frame = NSRect(x: curX, y: (rowHeight - 22) / 2, width: 68, height: 22)
        addSubview(seg)
        curX += 68
        return curX
    }

    private func addCensorStyleSegment(at x: CGFloat, ov: OverlayView) -> CGFloat {
        var curX = x
        let seg = NSSegmentedControl()
        seg.segmentCount = 2
        seg.trackingMode = .selectOne
        ...
        (seg.cell as? NSSegmentedCell)?.segmentStyle = .roundRect
        seg.setImage(NSImage(systemSymbolName: "circle.grid.2x2", accessibilityDescription: L("Pixelate")), forSegment: 0)
        seg.setImage(NSImage(systemSymbolName: "drop.fill", accessibilityDescription: L("Blur")), forSegment: 1)
        seg.setWidth(34, forSegment: 0)
        seg.setWidth(34, forSegment: 1)
        let mode = editingAnnotation?.censorMode
            ?? CensorMode(rawValue: UserDefaults.standard.integer(forKey: "censorMode"))
            ?? .pixelate
        seg.selectedSegment = mode == .blur ? CensorMode.blur.rawValue : CensorMode.pixelate.rawValue
        seg.toolTip = "\(L("Pixelate")) / \(L("Blur"))"
        seg.frame = NSRect(x: curX, y: (rowHeight - 22) / 2, width: 68, height: 22)
        addSubview(seg)
        curX += 68
        return curX
    ...
    private func addCensorStrengthSlider(at x: CGFloat, ov: OverlayView) -> CGFloat {
        var curX = x
        let strength = CensorStrength.clamped(
            editingAnnotation?.censorStrength
                ?? CGFloat(UserDefaults.standard.object(forKey: "censorStrength") as? Double ?? Double(CensorStrength.defaultValue))
        )
        let slider = NSSlider(value: Double(strength), minValue: 1, maxValue: 10,
                              target: self, action: #selector(censorStrengthChanged(_:)))
        slider.isContinuous = true
        slider.numberOfTickMarks = 10
        slider.allowsTickMarkValuesOnly = true
        slider.frame = NSRect(x: curX, y: (rowHeight - 20) / 2, width: 96, height: 20)
        slider.toolTip = L("Strength")
        addSubview(slider)
        curX += 100

        let label = NSTextField(labelWithString: "\(Int(strength.rounded()))")
        label.font = NSFont.monospacedDigitSystemFont(ofSize: 12, weight: .medium)
        label.textColor = ToolbarLayout.iconColor
        label.alignment = .right
        label.frame = NSRect(x: curX, y: (rowHeight - 16) / 2, width: 20, height: 16)
        label.tag = 981
        addSubview(label)
        return curX + 20
    }

    private func rebakeCensor(_ annotation: Annotation, overlay: OverlayView) {
        annotation.bakedBlurNSImage = nil
        annotation.bakePixelate()
        overlay.cachedCompositedImage = nil
        overlay.needsDisplay = true
    }

    private func censorMode(for segment: Int) -> CensorMode {
        segment == CensorMode.blur.rawValue ? .blur : .pixelate
    }

    @objc private func censorShapeChanged(_ sender: NSSegmentedControl) {
        guard let shape = CensorShape(rawValue: sender.selectedSegment), let ov = overlayView else { return }
        UserDefaults.standard.set(shape.rawValue, forKey: "censorShape")
        ov.setCurrentCensorShape(shape)
        guard let ann = editingAnnotation, ann.tool == .pixelate || ann.tool == .blur else { return }
        ensureSnapshot()
        let rect = ann.boundingRect
        ann.censorShape = shape
        if shape == .brush {
            ann.points = [
                NSPoint(x: rect.minX + ann.strokeWidth / 2, y: rect.midY),
                NSPoint(x: rect.maxX - ann.strokeWidth / 2, y: rect.midY),
            ]
            ann.strokeWidth = max(4, rect.height)
        } else {
            ann.startPoint = rect.origin
            ann.endPoint = NSPoint(x: rect.maxX, y: rect.maxY)
            ann.points = nil
        }
        rebakeCensor(ann, overlay: ov)
    }

    @objc private func censorStrengthChanged(_ sender: NSSlider) {
        guard let ov = overlayView else { return }
        let strength = CensorStrength.clamped(CGFloat(sender.doubleValue))
        UserDefaults.standard.set(Double(strength), forKey: "censorStrength")
        ov.setCurrentCensorStrength(strength)
        if let ann = editingAnnotation, ann.tool == .pixelate || ann.tool == .blur {
            ensureSnapshot()
            ann.censorStrength = strength
            rebakeCensor(ann, overlay: ov)
        } else {
            ov.needsDisplay = true
        }
        if let label = viewWithTag(981) as? NSTextField {
            label.stringValue = "\(Int(strength.rounded()))"
        }
    }
    ...
    private func addStampOptions(at x: CGFloat, ov: OverlayView) -> CGFloat {
        ...
        moreBtn.tag = StampStyleControlTag.moreEmoji
        ...
        loadBtn.tag = StampStyleControlTag.image
    ...
    @objc private func censorModeChanged(_ sender: NSSegmentedControl) {
        let mode = censorMode(for: sender.selectedSegment)
        UserDefaults.standard.set(mode.rawValue, forKey: "censorMode")
        overlayView?.setCurrentCensorMode(mode)
        if let ann = editingAnnotation, let ov = overlayView, ann.tool == .pixelate || ann.tool == .blur {
            ensureSnapshot()
            ann.censorMode = mode
            rebakeCensor(ann, overlay: ov)
        }
    ...
    @objc private func boldToggled() { overlayView?.textEditor.toggleBold(); overlayView.map { $0.textEditor.resizeToFit(); $0.applyTextFormattingToSelectedAnnotations(); $0.needsDisplay = true; rebuild(for: $0.currentTool) } }
    @objc private func italicToggled() { overlayView?.textEditor.toggleItalic(); overlayView.map { $0.textEditor.resizeToFit(); $0.applyTextFormattingToSelectedAnnotations(); $0.needsDisplay = true; rebuild(for: $0.currentTool) } }
    @objc private func underlineToggled() { overlayView?.textEditor.toggleUnderline(); overlayView.map { $0.textEditor.resizeToFit(); $0.applyTextFormattingToSelectedAnnotations(); $0.needsDisplay = true; rebuild(for: $0.currentTool) } }
    @objc private func strikethroughToggled() { overlayView?.textEditor.toggleStrikethrough(); overlayView.map { $0.textEditor.resizeToFit(); $0.applyTextFormattingToSelectedAnnotations(); $0.needsDisplay = true; rebuild(for: $0.currentTool) } }
    ...
    @objc private func quickEmojiClicked(_ sender: NSButton) {
        ...
        ov.refreshStampStyleControls()
        ov.refreshCursorAtPointerIfInside()
        ov.needsDisplay = true
    }
