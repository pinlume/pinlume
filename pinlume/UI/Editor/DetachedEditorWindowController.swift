...
class DetachedEditorWindowController: NSObject, NSWindowDelegate {
    ...
    private var scrollObservationTokens: [NSObjectProtocol] = []
    ...
    private func show(image: NSImage, tool: AnnotationTool?, color: NSColor?, strokeWidth: CGFloat?, annotations: [Annotation]) {
        ...
        win.title = "Pinlume Editor — \(dateFmt.string(from: Date()))"
        ...
        if let strokeWidth = strokeWidth {
            view.currentStrokeWidth = AnnotationStrokeWidth.clamped(strokeWidth)
        }
        ...
        scrollObservationTokens.append(NotificationCenter.default.addObserver(
    ...
    private func removeScrollObservers() {
        for token in scrollObservationTokens {
            NotificationCenter.default.removeObserver(token)
        }
        scrollObservationTokens.removeAll()
    }
