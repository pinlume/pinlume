...
final class VideoEditorWindowController: NSObject, NSWindowDelegate {
    ...
    private func show(url: URL, deleteOnClose: Bool = true) {
        ...
        win.title = "\(url.deletingPathExtension().lastPathComponent) — \(L("Pinlume Video Editor"))"
...
private final class VideoEditorView: NSView {
    ...
    private func setupPlayer() {
        ...
        playerSetupTask = Task { [weak self, asset] in
            ...
            await MainActor.run { [weak self] in
                guard !Task.isCancelled, let self, self.asset === asset else { return }
    ...
    private func buildPlayerView() {
        ...
        overlay.onTextEditRequested = { [weak self, weak overlay] viewRect in
    ...
    func cleanup() {
        DiagnosticLogStore.append(
            "video editor resources release durationMs=\(Int(duration * 1_000)) "
                + "thumbnails=\(thumbnailImages.count) thumbnailStrip=\(thumbnailStrip == nil ? 0 : 1) "
                + "textRasters=\(textRasterCache.count) setupTask=\(playerSetupTask == nil ? 0 : 1) "
                + "thumbnailGenerator=\(thumbnailGenerator == nil ? 0 : 1)")
    ...
    private func exportEditedTemp(completion: @escaping (URL?) -> Void) {
        ...
        let temporaryURL = FileManager.default.temporaryDirectory
            .appendingPathComponent(UUID().uuidString + ".\(videoURL.pathExtension)")
    ...
    private func reencodeExport(
        ...
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            ...
            do {
                ...
                let videoQueue = DispatchQueue(label: "Pinlume.export.video")
                let audioQueue = DispatchQueue(label: "Pinlume.export.audio")
    ...
    private func uploadVideo() {
        ...
        if !needsExport {
            ...
            guard let asset = asset else { return }
            let tmpURL = FileManager.default.temporaryDirectory.appendingPathComponent("pinlume_upload_\(UUID().uuidString).mp4")
