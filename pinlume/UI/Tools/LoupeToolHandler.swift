...
/// A vector-only canvas can provide an annotation-only snapshot for the loupe
/// without ever sampling the desktop beneath a transparent overlay.
protocol AnnotationSourceImageProviding: AnyObject {
...
final class LoupeToolHandler: AnnotationToolHandler {
    ...
    func start(at point: NSPoint, canvas: AnnotationCanvas) -> Annotation? {
        ...
        if let sourceProvider = canvas as? AnnotationSourceImageProviding,
           let source = sourceProvider.annotationSourceImageForLoupe(at: point) {
