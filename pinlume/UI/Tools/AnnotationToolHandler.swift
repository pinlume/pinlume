...
protocol AnnotationCanvas: AnyObject {
    ...
    /// Pins can constrain a measurement to their image without also pulling
    /// its endpoint onto alignment-guide targets.
    var allowsMeasureSnapGuides: Bool { get }
    ...
    /// Starts an incrementally-mutated annotation layer for a live eraser stroke.
    func beginLiveEraser(at point: NSPoint)
    /// Applies one new eraser segment without rebuilding committed annotations.
    func eraseLiveAnnotationSegment(from: NSPoint, to: NSPoint, strokeWidth: CGFloat)
    /// Publishes the incrementally-mutated layer after the eraser is committed.
    func finishLiveEraser()

    /// Starts the one-shot editing session, when the committed tool supports one.
...
protocol AnnotationToolHandler {
    ...
    /// Release state that belongs to one capture/editor session. Stateless
    /// handlers keep the default no-op implementation.
    func resetSession()
}
...
extension AnnotationToolHandler {
    ...
    func commitAnnotation(_ annotation: Annotation, canvas: AnnotationCanvas) {
        ...
        if annotation.tool == .eraser {
            canvas.finishLiveEraser()
        } else if let prev = previousCache {
