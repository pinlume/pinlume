...
class RecordingHUDPanel: NSPanel {
    ...
    /// falls back to below if it would go off screen, then avoids camera chrome.
    func positionOnScreen(relativeTo screenRect: NSRect, screen: NSScreen?, avoiding: NSRect? = nil) {
        setFrameOrigin(RecordingHUDLayout.origin(
            hudSize: frame.size,
            recordingRect: screenRect,
