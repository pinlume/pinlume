...
extension OverlayView {
    ...
    func showUploadConfirmPopover(anchorRect: NSRect, anchorView: NSView? = nil) {
        ...
        let current = UploadConfirmation.isEnabled
    ...
    func showTranslatePopover(anchorRect: NSRect, anchorView: NSView? = nil) {
        ...
        // Do not probe every possible Apple language pair here. The actual
        // detected source → selected target pair is checked immediately before
        // translation, which avoids unrelated language-pack prompts.
        showPopover(nil)
    }
    ...
    func showEmojiPopover(anchorView: NSView? = nil, anchorRect: NSRect = .zero) {
        ...
        picker.onSelectEmoji = { [weak self] emoji in
            ...
            self?.refreshStampStyleControls()
            self?.refreshCursorAtPointerIfInside()
            self?.needsDisplay = true
        }
        if let anchor = anchorView {
            PopoverHelper.show(
