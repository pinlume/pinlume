...
enum AnnotationTool: Int, CaseIterable {
    ...
    case eraser          // removes existing annotations; never rendered itself
}
...
class Annotation {
    /// Draw annotations in their stable z-order onto a transparent layer.
    /// Destructive tools such as eraser therefore clear annotation pixels only;
    /// the screenshot is composited separately underneath this layer.
    static func drawOrdered(
        _ annotations: [Annotation],
        in context: NSGraphicsContext,
        drawHighlightDim: (() -> Void)? = nil
    ) {
    ...
    var censorShape: CensorShape = .rectangle
    var censorStrength: CGFloat = CensorStrength.defaultValue
    ...
    func copyProperties(from src: Annotation) {
        ...
        censorShape = src.censorShape
        censorStrength = src.censorStrength
        if tool == .loupe {
            loupeSourceRect = src.loupeSourceRect
            bakedBlurNSImage = nil
            bakeLoupe()
        } else if tool == .pixelate || tool == .blur {
            // Manipulation snapshots already contain the exact local result.
            // Reusing it keeps undo/redo valid after the locked annotation has
            // released its full-screen source image.
            bakedBlurNSImage = src.bakedBlurNSImage
            if bakedBlurNSImage == nil { bakePixelate() }
        }
    ...
    var boundingRect: NSRect {
        ...
        if (tool == .pixelate || tool == .blur), censorShape == .brush, let points, !points.isEmpty {
            return CensorBrushGeometry.expandedBounds(points: points, width: strokeWidth)
        }
    ...
    /// Bounds of the pixels a mark can actually occupy. `boundingRect` remains
    /// the construction geometry used by shape drawing, while this companion
    /// includes freehand path extrema and stroke width for crop/Pin geometry.
    var visibleBounds: NSRect {
        var rect = boundingRect
        if let points, !points.isEmpty {
            for point in points {
                rect = rect.union(NSRect(origin: point, size: .zero))
            }
        }
        if tool == .text, !textDrawRect.isEmpty {
            rect = rect.union(textDrawRect)
        }
        let outset: CGFloat
        switch tool {
        case .marker:
            outset = strokeWidth * 3
        case .pencil, .line, .arrow, .rectangle, .filledRectangle, .ellipse:
            outset = strokeWidth / 2
        case .loupe:
            outset = Annotation.loupeOutlineWidth / 2
        default:
            outset = 0
        }
    ...
    func draw(in context: NSGraphicsContext, freeformClipRect: NSRect? = nil) {
        NSGraphicsContext.current = context
    ...
    private func drawEraser() {
        guard let points, !points.isEmpty, let context = NSGraphicsContext.current else { return }
    ...
    private func drawFreeform(alpha: CGFloat, width: CGFloat, clipRect: NSRect?) {
        ...
        let drawableRanges: [Range<Int>]
        if let clipRect {
            drawableRanges = FreeformLiveRenderGeometry.intersectingPointRanges(
                points: points,
                clipRect: clipRect,
                strokeRadius: width / 2
            )
            guard !drawableRanges.isEmpty else { return }
        } else {
            drawableRanges = [0..<points.count]
        }
        ...
        if let pressures = pressures, pressures.count == points.count, lineStyle == .solid {
            ...
            // Draw only ranges touching the dirty region. All ranges share one
            // transparency layer so marker/pencil self-overlap keeps its opacity.
            for range in drawableRanges {
                for i in range {
    ...
    private func drawClippedDottedFreeform(
        points: [NSPoint],
        ranges: [Range<Int>],
        clipRect: NSRect,
        alpha: CGFloat,
        ...
        for range in ranges {
            ...
            var distanceUntilDot: CGFloat = 0
            for index in range.dropFirst() {
                ...
                var distance = distanceUntilDot
                while distance <= length {
                    let t = distance / length
                    let point = NSPoint(x: start.x + dx * t, y: start.y + dy * t)
                    let dotRect = NSRect(
                        x: point.x - radius,
                        y: point.y - radius,
                        width: width,
                    ...
                    if dotRect.intersects(clipRect) {
                        NSBezierPath(ovalIn: dotRect).fill()
                    }
                    distance += gap
                }
                distanceUntilDot = max(0, distance - length)
            }
        }
    ...
    func reRenderTextImage() {
        ...
        let newSize = TextAnnotationLayout.measure(mutable, fallbackFont: font)
        textDrawRect = TextAnnotationLayout.rect(
            preservingRotatedTopLeftOf: textDrawRect,
            newSize: newSize,
            rotation: rotation
        )
        textImage = TextAnnotationLayout.render(mutable, size: newSize)
    ...
    /// Produces the real pixelate/blur image for one canvas-space source region.
    /// The caller owns any shape mask; this deliberately returns a full rectangle.
    func renderCensorRegion(_ rect: NSRect) -> NSImage? {
        guard let sourceImage,
              let renderer = CensorRenderer(
                sourceImage: sourceImage,
                sourceBounds: sourceImageBounds
              )
        else { return nil }
    ...
    func bakePixelate() {
        ...
        guard let processed = renderCensorRegion(rect) else { return }

        guard censorShape == .brush else {
            bakedBlurNSImage = processed
            return
        }
        guard let points else { return }
        let localPoints = points.map { NSPoint(x: $0.x - rect.minX, y: $0.y - rect.minY) }
        guard let mask = CensorBrushGeometry.makeMask(size: rect.size, points: localPoints, width: strokeWidth) else { return }
        let masked = NSImage(size: rect.size, flipped: false) { drawRect in
            guard let cgContext = NSGraphicsContext.current?.cgContext else { return false }
            cgContext.saveGState()
            processed.draw(in: drawRect, from: .zero, operation: .copy, fraction: 1.0)
            cgContext.setBlendMode(.destinationIn)
            cgContext.draw(mask, in: drawRect)
            cgContext.restoreGState()
            return true
        }
        bakedBlurNSImage = masked
    }
    ...
    private func drawCensor(in context: NSGraphicsContext) {
        ...
        if censorShape == .brush, let points, !points.isEmpty {
            let path = NSBezierPath()
            path.lineCapStyle = .round
            path.lineJoinStyle = .round
            path.lineWidth = strokeWidth
            if points.count == 1, let point = points.first {
                ...
                (censorMode == .blur ? NSColor.white : NSColor.black).withAlphaComponent(0.35).setFill()
                path.fill()
            } else {
                path.move(to: points[0])
                for point in points.dropFirst() { path.line(to: point) }
                (censorMode == .blur ? NSColor.white : NSColor.black).withAlphaComponent(0.45).setStroke()
                path.stroke()
            }
            return
        }
    ...
    private func generateLoupeImage() -> NSImage? {
        ...
        let cropRect = NSRect(
            x: (srcX - bounds.minX) * scaleX,
            y: (srcY - bounds.minY) * scaleY,
    ...
    private func drawLoupe(in context: NSGraphicsContext) {
        ...
        if let baked = bakedBlurNSImage {
            ...
            let fromRect = NSRect(
                x: (sampleCenter.x - srcSize/2 - sourceImageBounds.minX) * scaleX,
                y: (sampleCenter.y - srcSize/2 - sourceImageBounds.minY) * scaleY,
                width: srcSize * scaleX,
                height: srcSize * scaleY
            )
