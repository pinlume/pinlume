...
/// Sweeps Pinlume-owned files from `NSTemporaryDirectory()` that match
...
///
/// Preserves:
///   - `Pinlume-clipboard.png` and `Pinlume-clipboard-recording.*`
...
private struct TmpFileCleaner: LaunchCleaner {
    ...
    /// Filename prefixes we know are ours. The `Pinlume-clipboard-`
    /// prefix here is intentional to catch the *legacy* UUID-named form
    /// from pre-fix builds; the current single-file `Pinlume-clipboard.png`
    ...
    private func isStale(name: String) -> Bool {
        ...
        if name == "Pinlume-clipboard.png" { return false }
        if name.hasPrefix("Pinlume-clipboard-recording.") { return false }
...
/// Sweeps everything in `tmp/Pinlume-share/` older than 5 minutes.
...
/// Sweeps the legacy `/tmp/Pinlume-clipboard/` folder used by older builds.
...
private struct LegacyClipboardTmpDirectoryCleaner: LaunchCleaner {
    ...
    func sweep() -> DirectorySweeper.Result {
        return DirectorySweeper.sweep(
            directory: FileManager.default.temporaryDirectory.appendingPathComponent("Pinlume-clipboard"),
            olderThan: ttl,
            shouldDelete: { _ in true }
        )
    }
