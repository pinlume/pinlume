...
enum ImageEncoder {
    ...
    static func fileExtension(for format: Format) -> String {
        switch format {
        case .png: return "png"
    ...
    static func utType(for format: Format) -> UTType {
        switch format {
        case .png: return .png
        case .jpeg: return .jpeg
        case .heic: return .heic
    ...
    static func encode(_ image: NSImage) -> Data? {
        encode(image, format: format)
    }

    static func encode(_ image: NSImage, format: Format) -> Data? {
    ...
    /// Encode PNG using the source image's pixel dimensions, independent of the
    /// user's display-resolution preference. Used by local persistence paths.
    static func encodePNGPreservingSourcePixels(_ image: NSImage) -> Data? {
        guard let bitmap = makeBitmap(image, downscale: false) else { return nil }
        return encodePNG(bitmap: bitmap)
    }

    /// Return the pixel dimensions of the image backing representation.
