...
private let hotkeyLog = OSLog(subsystem: AppIdentity.bundleIdentifier, category: "hotkey-timing")

extension Notification.Name {
    static let transientInteractionMustYield = Notification.Name(
        "Pinlume.transientInteractionMustYield")
}
...
class HotkeyManager {
    ...
    enum HotkeySlot: Int, CaseIterable {
        ...
        case toggleAllPins = 13
        case selectableOCRCapture = 14
        case translationWindow = 15
        case screenTranslationCapture = 16
        case transparentAnnotation = 17
        case presentationDrawing = 18
        ...
        var keyCodeKey: String {
            ...
            case .toggleAllPins: return "hotkeyToggleAllPinsKeyCode"
            case .selectableOCRCapture: return "hotkeySelectableOCRKeyCode"
            case .translationWindow: return "hotkeyTranslationWindowKeyCode"
            case .screenTranslationCapture: return "hotkeyScreenTranslationKeyCode"
            case .transparentAnnotation: return "hotkeyTransparentAnnotationKeyCode"
            case .presentationDrawing: return "hotkeyPresentationDrawingKeyCode"
            }
        }
        ...
        var modifiersKey: String {
            ...
            case .toggleAllPins: return "hotkeyToggleAllPinsModifiers"
            case .selectableOCRCapture: return "hotkeySelectableOCRModifiers"
            case .translationWindow: return "hotkeyTranslationWindowModifiers"
            case .screenTranslationCapture: return "hotkeyScreenTranslationModifiers"
            case .transparentAnnotation: return "hotkeyTransparentAnnotationModifiers"
            case .presentationDrawing: return "hotkeyPresentationDrawingModifiers"
            }
        }
        ...
        var defaultKeyCode: UInt32 {
            UInt32(SettingsProfileHotkeyDefinition.definition(for: rawValue)?.defaultKeyCode ?? 0)
        }
    ...
    private var registrationStatuses: [HotkeySlot: OSStatus] = [:]
    ...
    @discardableResult
    func register(slot: HotkeySlot, callback: @escaping () -> Void) -> OSStatus {
        ...
        guard modifiers != 0 || Self.isFunctionKey(keyCode) else {
            registrationStatuses[slot] = noErr
            return noErr
        }  // no modifiers = disabled (unless function key)
        ...
        registrationStatuses[slot] = status
        return status
    }

    func registrationStatus(for slot: HotkeySlot) -> OSStatus? {
        registrationStatuses[slot]
    }

    /// Register all hotkeys with their callbacks.
    func registerAll(captureArea: @escaping () -> Void, captureFullScreen: @escaping () -> Void, recordArea: @escaping () -> Void, recordScreen: @escaping () -> Void, historyOverlay: @escaping () -> Void, captureOCR: @escaping () -> Void, selectableOCRCapture: @escaping () -> Void, translationWindow: @escaping () -> Void, screenTranslationCapture: @escaping () -> Void, quickCapture: @escaping () -> Void, scrollCapture: @escaping () -> Void, openFromClipboard: @escaping () -> Void, captureLastArea: @escaping () -> Void, pinFromClipboard: @escaping () -> Void, toggleAllPins: @escaping () -> Void, clearHistory: @escaping () -> Void, transparentAnnotation: @escaping () -> Void, presentationDrawing: @escaping () -> Void) {
        unregisterAll()
        ...
        register(slot: .selectableOCRCapture, callback: selectableOCRCapture)
        register(slot: .translationWindow, callback: translationWindow)
        register(slot: .screenTranslationCapture, callback: screenTranslationCapture)
        ...
        register(slot: .toggleAllPins, callback: toggleAllPins)
        register(slot: .clearHistory, callback: clearHistory)
        register(slot: .transparentAnnotation, callback: transparentAnnotation)
        register(slot: .presentationDrawing, callback: presentationDrawing)
    }
    ...
    /// Restore every global shortcut to the shared profile defaults.
    ...
    static func isFunctionKey(_ keyCode: UInt32) -> Bool {
        SettingsProfileHotkeyDefinition.functionKeyCodes.contains(Int(keyCode))
    }
