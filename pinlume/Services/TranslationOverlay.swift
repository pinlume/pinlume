...
enum TranslateOverlay {
    ...
    static func translate(
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            VisionOCR.performTextRecognition(cgImage: cgImage) { request, error in
                ...
                let sourceLanguage = TranslationTextProcessor.detectedSourceLanguage(
                    for: blocks.map(\.text).joined(separator: "\n"))
                TranslationService.translateBatch(
                    texts: blocks.map { $0.text },
    ...
    static func sampleAverageColor(in cgImage: CGImage, region: CGRect) -> NSColor {
