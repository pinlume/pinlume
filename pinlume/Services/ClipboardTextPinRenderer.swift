...
enum ClipboardTextPinRenderer {
    ...
    static func render(_ attributed: NSAttributedString, fallbackBackground: NSColor = .white) -> NSImage? {
        ...
        let maxContentWidth = min(defaultTextPinWidth - padding.left - padding.right, screenFrame.width * 0.72)
        ...
        var imageWidth = max(defaultTextPinWidth, ceil(contentSize.width + padding.left + padding.right))
