...
/// Files are uploaded to a "Pinlume" folder in the user's Drive, kept private (not shared).
final class GoogleDriveUploader: NSObject, ASWebAuthenticationPresentationContextProviding {
    ...
    private var pinlumeFolderID: String?
    ...
    /// Upload a file (image or video) to the Pinlume folder.
    func upload(data: Data, filename: String, mimeType: String, completion: @escaping (Result<String, Error>) -> Void) {
        ensureValidToken { [weak self] success in
            ...
            self.ensurePinlumeFolder { result in
                switch result {
    ...
    private func ensurePinlumeFolder(completion: @escaping (Result<String, Error>) -> Void) {
        if let id = pinlumeFolderID { completion(.success(id)); return }
        ...
        // Search for existing Pinlume folder
        let query = "name='Pinlume' and mimeType='application/vnd.google-apps.folder' and trashed=false"
        ...
        URLSession.shared.dataTask(with: request) { [weak self] data, response, error in
            ...
            if let existing = files.first, let id = existing["id"] as? String {
                self.pinlumeFolderID = id
                DispatchQueue.main.async { completion(.success(id)) }
            } else {
                self.createPinlumeFolder(token: token, completion: completion)
            }
        }.resume()
    }

    private func createPinlumeFolder(token: String, completion: @escaping (Result<String, Error>) -> Void) {
    ...
    private func uploadFileWithRetry(data fileData: Data, filename: String, mimeType: String, folderID: String, attempt: Int, completion: @escaping (Result<String, Error>) -> Void) {
        ...
        let tmpFile = FileManager.default.temporaryDirectory.appendingPathComponent("pinlume_upload_\(UUID().uuidString).tmp")
